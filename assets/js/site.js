/* Davi Rodrigues Fotografia — comportamento das paginas */

(function () {
  'use strict';

  /* Revelacao ao rolar. Elementos com .rev sobem e aparecem uma unica vez. */
  function initReveal() {
    var targets = document.querySelectorAll('.rev');
    if (!targets.length) return;

    if (!('IntersectionObserver' in window)) {
      targets.forEach(function (el) { el.classList.add('in'); });
      return;
    }
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (e.isIntersecting) {
          e.target.classList.add('in');
          io.unobserve(e.target);
        }
      });
    }, { rootMargin: '0px 0px -12% 0px' });

    targets.forEach(function (el) { io.observe(el); });
  }

  /* FAQ: uma pergunta por vez, com a proxima espiando pela borda. */
  function initFaq() {
    var track = document.getElementById('faq-track');
    if (!track) return;

    var total = track.children.length;
    var idx = 0;
    var iEl = document.getElementById('faq-i');
    var nEl = document.getElementById('faq-n');
    var prev = document.getElementById('faq-prev');
    var next = document.getElementById('faq-next');
    if (nEl) nEl.textContent = total;

    function render() {
      /* cada slide ocupa 88% da largura do track, entao o passo e 88% */
      track.style.transform = 'translateX(' + (-idx * 88) + '%)';
      if (iEl) iEl.textContent = idx + 1;
      if (prev) prev.disabled = (idx === 0);
      if (next) next.disabled = (idx === total - 1);
    }

    if (prev) prev.addEventListener('click', function () { if (idx > 0) { idx--; render(); } });
    if (next) next.addEventListener('click', function () { if (idx < total - 1) { idx++; render(); } });

    /* setas do teclado quando o carrossel esta em foco */
    track.closest('.ab-faq').addEventListener('keydown', function (ev) {
      if (ev.key === 'ArrowLeft' && idx > 0) { idx--; render(); }
      if (ev.key === 'ArrowRight' && idx < total - 1) { idx++; render(); }
    });

    render();
  }

  /* Filtros do portfolio. Sem recarregar a pagina e sem quebrar o mosaico. */
  function initPortfolio() {
    var grid = document.getElementById('pf-grid');
    if (!grid) return;

    var buttons = document.querySelectorAll('.pf-filters button');
    var items = grid.querySelectorAll('.pf-item');
    var empty = document.getElementById('pf-empty');

    function apply(cat) {
      var shown = 0;
      items.forEach(function (it) {
        var match = (cat === 'todos' || it.dataset.cat === cat);
        it.hidden = !match;
        if (match) shown++;
      });
      if (empty) empty.hidden = (shown > 0);

      buttons.forEach(function (b) {
        b.setAttribute('aria-pressed', String(b.dataset.filter === cat));
      });
    }

    buttons.forEach(function (b) {
      b.addEventListener('click', function () { apply(b.dataset.filter); });
    });
  }

  /* Contato. Sem servidor: o formulário monta uma mensagem de WhatsApp já
     preenchida. O lead chega onde o Davi já atende, sem depender de e-mail
     nem de serviço de terceiro. */
  function initContact() {
    var form = document.getElementById('ct-form');
    if (!form) return;

    var WHATSAPP = '5512988275053';
    var tabs = document.querySelectorAll('.ct-tabs button');
    var tipo = 'Casamento';

    tabs.forEach(function (b) {
      b.addEventListener('click', function () {
        tipo = b.dataset.tipo;
        tabs.forEach(function (x) { x.setAttribute('aria-pressed', String(x === b)); });
      });
    });

    function dataPorExtenso(iso) {
      if (!iso) return '';
      var p = iso.split('-');
      if (p.length !== 3) return iso;
      return p[2] + '/' + p[1] + '/' + p[0];
    }

    form.addEventListener('submit', function (ev) {
      ev.preventDefault();

      /* validação própria: novalidate no form evita o balão nativo,
         que destoaria do resto da página */
      var faltando = [];
      ['nome', 'cidade', 'data'].forEach(function (n) {
        var f = form.elements[n];
        if (!f.value.trim()) { faltando.push(f); }
      });
      if (faltando.length) {
        faltando.forEach(function (f) { f.style.borderBottomColor = 'currentColor'; });
        faltando[0].focus();
        return;
      }

      var d = new FormData(form);
      var linhas = ['Oi, Davi! Vim pelo site.', ''];
      linhas.push(tipo + ' em ' + dataPorExtenso(d.get('data')));
      linhas.push('Cidade: ' + d.get('cidade'));
      linhas.push('Nome: ' + d.get('nome'));
      if (d.get('email'))     { linhas.push('E-mail: ' + d.get('email')); }
      if (d.get('instagram')) { linhas.push('Instagram: ' + d.get('instagram')); }
      if (d.get('origem'))    { linhas.push('Conheci por: ' + d.get('origem')); }
      if (d.get('mensagem'))  { linhas.push('', d.get('mensagem')); }

      window.open(
        'https://wa.me/' + WHATSAPP + '?text=' + encodeURIComponent(linhas.join('\n')),
        '_blank', 'noopener'
      );
    });
  }

  function init() {
    initReveal();
    initFaq();
    initPortfolio();
    initContact();
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', init);
  } else {
    init();
  }
})();
